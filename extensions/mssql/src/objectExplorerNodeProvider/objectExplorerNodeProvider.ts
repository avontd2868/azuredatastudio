...
export class SqlObjectExplorerNodeProvider extends ProviderBase implements sqlops.ObjectExplorerNodeProvider, ITreeChangeHandler {
	...
	private async notifyNodeChangesAsync(node: TreeNode): Promise<void> {
		try {
			let session = this.getClusterSessionForNode(node);
			if (!session) {
	...
	private getClusterSessionForNode(node: TreeNode): SqlClusterSession {
	...
	async findNodeForSqlContext<T extends TreeNode>(sqlContext: sqlops.ObjectExplorerContext): Promise<T> {
		let node: T = undefined;
		let sqlConnProfile = sqlContext.connectionProfile;
		let session = this.findClusterSessionBySqlConnProfile(sqlConnProfile);
		if (session) {
			if (sqlContext.isConnectionNode) {
				...
				node = <T><any>await session.rootNode.findNodeByPath(sqlContext.nodeInfo.nodePath, true);
			}
		}
		return node;
	}

	public findClusterSessionBySqlConnProfile(sqlConnProfile: sqlops.IConnectionProfile): SqlClusterSession {
